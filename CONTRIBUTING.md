...
| `이 라인(staging/X.Y)에는 설치 정합성 검사가 아직 없습니다` | 그 staging 라인에 아직 검사 스크립트가 머지되지 않았다(이식 과도기) | 정상입니다. 이번 머지로 들어가면 다음 배포부터 검사가 돕니다 — 다만 이번 배포는 스큐를 못 잡으니 타입 오류가 나면 라인 차이를 의심하세요 |
| `설치 상태 문제가 아닙니다 — yarn install 로는 해결되지 않습니다` | 설치 스큐가 아니라 검사기 자체가 실패했다(문법 오류·예외 등) | `node scripts/check-install-sync.mjs` 를 직접 실행해 원인을 확인하세요. `yarn install` 은 무관합니다 |
| `원격이 push 를 거부했습니다` | 로컬 검사는 통과했고 원격이 막았다(브랜치 보호 규칙·서버 훅·권한) | 위 remote 출력을 확인하고 원격 설정을 해결한 뒤 같은 명령을 다시 실행하세요. staging 라인은 되돌려져 있습니다 |
